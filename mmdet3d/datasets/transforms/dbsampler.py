...
class DataBaseSampler(object):
    ...
    def sample_one(self,
        ...
        Returns:
            dict: Dict of sampled 'pseudo ground truths'.
                ...
                - group_ids (np.ndarray): ids of sampled ground truths
        """
        sample_num_per_class = [10] # ---> Pedestrian only one object
        ...
        if len(sampled) > 0:
            ...
            lengths = [p.tensor.shape[0] for p in s_points_list]
            cum = np.cumsum([0] + lengths)  # [0, n0, n0+n1, ...]
            obj_slices = [(cum[i], cum[i+1]) for i in range(len(lengths))]

            ret = {
                'gt_labels_3d': gt_labels,
                'gt_bboxes_3d': sampled_gt_bboxes,
                'points': s_points_list[0].cat(s_points_list),  # 전체 합본
                'obj_slices': obj_slices,                 # ✅ (start, end) per object
                'db_infos': sampled,
    ...
    def sample_idx(self,
        ...
        ped_sampler = self.sampler_dict['pedestrian']
        target_path = db_path
        
        paths = [s['path'] for s in ped_sampler._sampled_list]
        paths = np.array(paths)
        idx = np.where(paths == target_path)[0]
        
        sample = ped_sampler._sampled_list[idx[0]]
        ...
        file_path = os.path.join(
                    self.data_root,
                    sample['path']) if self.data_root else sample['path']
        results = dict(lidar_points=dict(lidar_path=file_path))
        s_points = self.points_loader(results)['points']
        s_points.translate(sample['box3d_lidar'][:3])
        
        s_points_list.append(s_points)
        gt_labels = np.array(0, dtype=np.long)
        sampled_gt_box = sample['box3d_lidar'][
                            np.newaxis, ...]
